...
def get_colors():
    ...
    -------  
    node_color : array
        Colors of node (RGB + transparency)
    """
...
def get_labels(G, attribute):
    """ Get colors for plotting node attributes
...
def get_edge_labels(G, attribute):
    """ Get colors for plotting edge attributes
...
def min_but_0(G, attribute):
    """ Minimum but 0 in attribute
    ...
    -------  
    min(values) : float
        minimum but 0 of the attribute
    """

    # Assertions
    assert isinstance(G, nx.Graph), "G is not a NetworkX graph"   
    assert (attribute in G.edges[list(G.edges(0))[0]]) == True , \
    'The attribute '+attribute+' is not in edges of the Graph. Check that you have already computed the attribute.'

    values = []
    for n, edge in enumerate(G.edges()):
        if G.edges[edge][attribute]!=0:
            values.append(G.edges[edge][attribute])
    return min(values)
